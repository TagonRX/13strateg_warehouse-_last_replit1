...
*   **Location Management**: Administrators can mass-upload locations and barcodes via CSV files, supporting both comma and semicolon delimiters, automatic header detection, and preservation of existing barcodes. Features "Download Template" button and performance optimizations for large datasets. Individual location barcode edits are automatically persisted to database via PATCH mutation, making them immediately available in placement workflow without requiring manual save button click. Optimized with memoized LocationsManagementTable component and debounced range filters to prevent input lag.
