...
- ✅ **Faulty Stock Protection**: Added critical validation to prevent Faulty/Parts items from being added to inventory. System now checks `faultyStock` table and blocks stock-in attempts with clear error message: "ЗАПРЕЩЕНО! Товар отмечен как неисправный (Faulty/Parts) и НЕ МОЖЕТ быть добавлен на склад." Includes `isFaulty: true` flag for frontend error handling. Implemented `getFaultyStockByBarcode()` method in storage layer.
