...
*   **Barcode Scanner Auto-Focus**: USB scanner mode features intelligent global auto-focus that automatically focuses the barcode input field on mount and when user clicks empty areas, while preserving focus on interactive controls (buttons, inputs, selects). Event-driven focus management prevents scanner input from being lost when other fields are in focus.
