...
*   **Tables**: `users`, `inventory_items` (with `price` field for cost tracking), `event_logs` (comprehensive audit with full product tracking, `quantity` and `price` fields for cost analytics), `worker_analytics`, `picking_lists`, `picking_tasks` (including `itemName`), `sku_errors`, `csv_sources` (globally shared CSV sources), `global_settings` (shared configuration like CSV credentials), `pending_tests` (products in testing with first scan timestamp), `tested_items` (tested products with condition), `faulty_stock` (faulty items with working hours analytics).
*   **Design Decisions**: UUID primary keys, automatic timestamps, foreign key relationships, product deduplication by `productId`, upsert patterns for bulk inventory updates. Event logs store complete product information including quantity and price for cost traceability. CSV sources are globally shared across all users for consistent picking list management. Testing workflow uses separate tables for pending/tested/faulty items with automatic cleanup on inventory placement.
...
*   **Scanner Mode Page**: Dedicated "/scanner-mode" page for mobile devices to work exclusively as remote barcode scanner. Features camera-based barcode scanning with Html5Qrcode, quantity input, live WebSocket connection status indicator, and automatic barcode transmission to connected computer. Accessible from sidebar menu for all users (worker/admin). Designed for dual-device workflow where computer runs main application and mobile phone acts as wireless scanner only.
...
*   **Product Testing Workflow**: Two-phase testing system for incoming products with dual-mode barcode scanning (USB/Phone). First scan starts testing session (adds to pending_tests table with timestamp and worker ID). Second scan prompts condition selection (Used/Exdisplay/New/Parts/Faulty) with confirmation button. Faulty items routed to dedicated Faulty Stock page (admin-only) with analytics showing: first scan date/user, decision date/user, and working hours between scans (calculated at 8h/day, 5 days/week excluding weekends). Non-faulty items stored in tested_items list with condition tag. Automatic cleanup: when tested item scanned during Stock-In (location placement) or Inventory verification, item auto-removed from pending/tested lists and placed in regular inventory. Working hours calculation accounts for business hours only (Mon-Fri, 8h/day) with proper multi-day handling. Dual scanner modes: USB mode with zero-leak routing via useGlobalBarcodeInput hook, Phone mode with WebSocket connection and live connection indicator (green=connected, amber=connecting). Scanner mode selector persists throughout testing workflow.
