...
*   **Packing Workflow**: Multi-worker order fulfillment with **shipping label-based order lookup**. System searches by priority: shipping label → item barcode → SKU. Orders display as `orderNumber__shippingLabel` format for easy identification. Supports concurrent packing sessions with real-time updates. Displays pending orders reference table at bottom for read-only access.
