...
*   **Barcode Scanner Workflow**: USB scanner mode with intelligent focus management - global keydown listener captures scanner input even when barcode field is not focused, automatically routing it to the correct input. Manual confirmation required: after scanning, user must click "Добавить товар" button (no auto-submit). Focus remains where user places it (buttons, inputs, selects) without interference, while scanner input is captured seamlessly. Barcode field properly persists to database with price tracking and is fully searchable in inventory table filter.
