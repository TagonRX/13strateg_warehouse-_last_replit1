...
*   **Multiple CSV Sources Management**: Added support for managing multiple CSV data sources with independent enable/disable toggles. Each source stores URL, username, password, and enabled state in localStorage. Features include: (1) Add/remove sources dynamically with "Add Source" button, (2) Green/gray toggle switches to enable/disable individual sources, (3) "Load All" functionality to fetch data from all enabled sources in parallel, (4) Automatic SKU deduplication and quantity merging across sources, (5) Simplified CSV auto-detection (SKU, name, quantity columns) without complex field mapping UI. Sources persist across sessions and can be independently managed.
